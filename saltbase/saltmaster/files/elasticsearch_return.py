{%- set envshort = grains['domain'].replace(".dpcops.com", "") -%}
{% raw %}# -*- coding: utf-8 -*-
'''
...
{% endraw %}
    index = 'salt-{{ envshort }}-{0}-{1}'.format(job_fun_escaped, datetime.today().date().strftime('%Y.%m.%d')) #TODO prefer this? #TODO make it configurable!
{% raw %}
    ...
    if not index_exists:
        ...
        __salt__['elasticsearch.index_create']('{0}'.format(index), index_definition)
        __salt__['elasticsearch.alias_create']('{0}'.format(index), index)
