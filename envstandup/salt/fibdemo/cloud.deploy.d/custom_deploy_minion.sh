#!/bin/bash

SALTMASTER=$1
POD=$2
ROLES=$3
VMHOSTNAME=$(cat /etc/hostname)
cat << EOF > /tmp/saltstack.pub
-----BEGIN PGP PUBLIC KEY BLOCK-----
Version: GnuPG v2.0.22 (GNU/Linux)


=dtMN
-----END PGP PUBLIC KEY BLOCK-----
EOF

cat /tmp/saltstack.pub | sudo apt-key add -
sudo sh -c "echo 'deb http://repo.saltstack.com/apt/ubuntu/14.04/amd64/latest trusty main' >> /etc/apt/sources.list"
sudo apt-get update && sudo apt-get -y -o Dpkg::Options::=--force-confdef -o Dpkg::Options::=--force-confnew install salt-minion

cat << EOF > /tmp/salt-minion-grains
grains:
  roles:
    - ${ROLES}
EOF
sudo mv /tmp/salt-minion-grains /etc/salt/minion.d/grains.conf

cat << EOF > /tmp/salt-minion
master: ${SALTMASTER}
EOF
sudo mv /tmp/salt-minion /etc/salt/minion.d/master.conf

echo "===== restarting minion"
sudo service salt-minion restart

echo "===== sleep 45 to give the salt-minion restart time to highstate"
sleep 45

echo "===== updating saltminion first"
sudo salt-call state.sls saltminion

echo "===== restarting saltminion after changes"
sudo service salt-minion restart

echo "===== getting our users setup" 
sudo salt-call state.sls users

echo "===== k now we can highstate"
sudo salt-call state.highstate
